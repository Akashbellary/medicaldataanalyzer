...
import config  # Import external config file for API keys

def set_environment_variables():
    """Set necessary environment variables (handled by config.py)."""
    if not os.getenv("NVIDIA_API_KEY"):
        raise ValueError("NVIDIA_API_KEY is not set in config.py")
...
def process_graph(image_content):
    ...
    mixtral = NVIDIA(model="meta/llama-3.1-70b-instruct")  # Updated to your specified model
...
def describe_image(image_content):
    ...
    if not api_key:
        raise ValueError("NVIDIA API Key is not set. Please set it in config.py.")
